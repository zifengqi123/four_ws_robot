...
`inverted_laser (string, default: "false")` - true если сканы формируются против часовой стрелки
...
`kernelSize (int, default: 1)` - определяет размер ядра, используемого в операции сглаживания карты (чем оно больше - тем больше размытие)
