...
`[sensor]_differential` - позволяет указать, нужно ли дифференцировать переменные позиции для сообщений датчика, содержащих информацию о позе. Если этот параметр установлен в `true` для конкретного датчика, то измерения позиции в момент времени `t` будут преобразованы в скорость, вычитая измерение позиции в момент времени `t-1`. Это полезно, когда у робота есть два источника информации об абсолютной позиции, например, данные от одометра и IMU, и если эти данные имеют небольшие отклонения и могут вызывать колебания в фильтре. Дифференцирование и интегрирование этих измерений различным образом позволяет избежать этой проблемы.
Однако, при использовании этого параметра для данных ориентации следует быть осторожным, так как преобразование в скорость приведет к неограниченному росту ковариации для переменных состояния ориентации (если не используется другой источник абсолютной ориентации). Если ваша цель состоит просто в том, чтобы все переменные позиции начинались с `0`, рекомендуется использовать параметр `[sensor]_relative` вместо этого.

`[sensor]_relative` - если для этого параметра установлено значение `true`, то все измерения с этого датчика будут объединены относительно первого измерения, полученного с этого датчика. Это полезно, если, например, вы хотите, чтобы ваша оценка состояния всегда начиналась с (0,0,0) и со значениями крена, тангажа и рыскания, равными (0,0,0). Это аналогично параметру `[sensor]_differential`, но вместо удаления измерения в момент времени `t−1` мы всегда удаляем измерение в момент времени `0`, и измерение не преобразуется в скорость
